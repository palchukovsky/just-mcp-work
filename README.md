...
reads build files instead of evaluating them, so a target list is never a
promise of completeness - most visibly for Make, where a build file outside the
safe literal subset reports a discovery error instead of a partial list. Need
